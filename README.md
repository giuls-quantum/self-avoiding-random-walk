...
- O(1) collision detection via a pre-allocated boolean grid instead of array scanning.
- Faster and higher-quality randomness through an Xorshift32 PRNG instead of the standard C rand() function.
...
- Saves simulation data in .dat files:
  ...
  - averages.dat – average distance from the origin over multiple walks.
...
The program will generate .dat files in the data/ folder and PDF plots in the plots/ folder (if gnuplot is installed).

## Output

- positions.dat – trajectory of the first simulated walk.
- averages.dat – average squared distance vs. attempt number.
- distance.dat – short-time squared distance from the origin.
